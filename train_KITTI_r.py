...
def main():
    ...
    file_dir = Path(str(experiment_dir) + '/%s-KITTI_r-'%args.model_name + str(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M')))
    ...
    init_epoch = 0
    lr_lambda = lambda epoch: 1.0 if epoch < 200 else 0.1
...
def eval_sceneflow(model, loader):
    ...
    for index_test in range(10):  # Evaluate model multiple times to obtain stable metric results
